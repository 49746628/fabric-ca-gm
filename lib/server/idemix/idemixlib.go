...
type Lib interface {
	...
	CreateCRI(key interface{}, unrevokedHandles []*fp256bn.BIG, epoch int, alg idemix.RevocationAlgorithm, rng *amcl.RAND) (cri *idemix.CredentialRevocationInformation, err error)
	GenerateLongTermRevocationKey() (pk interface{}, err error)
...
func (i *libImpl) GenerateLongTermRevocationKey() (pk interface{}, err error) {
	defer func() {
		r := recover()
		if r != nil {
			err = errors.Errorf("failure: %s", r)
